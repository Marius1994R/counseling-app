...
import { COMMON_SPECIALTIES, getWorkloadLabel } from '../Profile/profileUtils';
...
export function resolveSpecialtyIssueType(
  ...
  if (COMMON_SPECIALTY_ISSUE_TYPES[specialty]) {
    return COMMON_SPECIALTY_ISSUE_TYPES[specialty];
  }
...
export function isCommonSpecialty(specialty: string): boolean {
  return (COMMON_SPECIALTIES as readonly string[]).includes(specialty);
}
